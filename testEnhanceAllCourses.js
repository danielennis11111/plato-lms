...
async function enhanceAllCourses() {
  ...
  try {
    // Import the mock Canvas API dynamically
    const { mockCanvasApi } = await import('./src/lib/mockCanvasApi.ts');
...
function generateModuleItems(moduleNumber, topic, department, startDate) {
  ...
  items.push({
    ...
    type: 'quiz',
    content: `Comprehensive assessment covering ${topic?.title?.toLowerCase() || 'module content'}`,
...
function generateAssignmentContent(topic, department) {
